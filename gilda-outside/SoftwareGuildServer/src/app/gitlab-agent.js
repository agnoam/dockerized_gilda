...
class GitLabAgent {
    constructor() {
        ...
        this.newUser$ = new Subject_1.Subject();
        this.projectsUpdate$ = new Subject_1.Subject();
        ...
        this.newProject$ = new Subject_1.Subject();
    ...
    getProjectIssue$(project_id, issue_iid) {
        return rp.get(this.apiUrl + '/projects/' + project_id + '/issues/' + issue_iid, { json: true });
    }
    setIssueDuration(project_id, issue_id, hours) {
        return rp.post(this.apiUrl +
            '/projects/' +
            project_id +
            '/issues/' +
            issue_id +
            ...
            hours +
            'h&' +
            this.private_token, { json: true });
    }
    updateIssue(project_id, issue_id, assignee_id, weight = 0, labels = '') {
    ...
    addIssue(project_id, title, confidential = false, description = '', weight = 0, labels = '') {
    ...
    handleNewProject(project_id) {
        this.getProject(project_id).then((project) => this.addGroup2ProjectDevelopers(project, this.allUsersGroupId));
    ...
    createLabel(label, description, project_id, color = this.getRandomColor()) {
        let options = { method: 'POST',
    ...
    addAllUsersAsDevelopersofAllProjects(group_id) {
        this.getProjects$()
            .subscribe((projects) => projects.forEach((project) => this.addGroup2ProjectDevelopers(project, group_id), this));
    }
    addGroup2ProjectDevelopers(project, group_id) {
        let shared_groups = project.shared_with_groups;
        if (shared_groups.findIndex(x => x.group_id == group_id) == -1) {
            if (project.visibility == 'public') {
                rp.post(this.apiUrl + '/projects/' + project.id + '/share/?group_id=' + group_id + '&group_access=30&' + this.private_token)
                    .then(() => { }, (err) => logger.error(err.message + ' ' + project.id + ' ' + project.name));
            }
        }
        else {
    ...
    addSystemHooks() {
        let hookUrl = 'http://' + ip.address() + ':' + process.env.PORT + '/systemeventslistener';
        rp.get(this.apiUrl + '/hooks?' + this.private_token)
            .then((hooks) => {
            let myHooks = JSON.parse(hooks).filter((hook) => hook.url == hookUrl);
            if (myHooks.length == 0) {
                let options = { method: 'POST',
                ...
                rp(options)
                    .then((res) => logger.info(res), err => { logger.error(err); });
            }
        }).catch(() => { });
    }
    setUserAvater(user_id) {
        ...
        let options = { method: 'PUT',
            ...
            qs: { private_token: process.env.GITLAB_PRIVATE_TOKEN },
            formData: { avatar: { value: fs.createReadStream(filename),
                    options: { filename: filename }
                } } };
        return rp(options);
        //});
    }
    getProjectMattermostService(project_id) {
        return rp.get({
    ...
    getGroupMembers(group) {
        return new Promise((resolve, reject) => {
            this.getListFromGitLabServer$(this.apiUrl + '/groups/' + group.id + '/members', '')
                .subscribe((users) => {
                users.forEach((user) => group.users.push(user.id), this);
                resolve(group);
    ...
    getGroupProjects(group) {
        return new Promise((resolve, reject) => {
            rp.get({
                ...
                .then((groupData) => {
                groupData.projects.forEach((project) => group.projects.push(project.id), this);
                resolve(group);
            }, err => { logger.error(err + group.name); });
        });
    }
    ...
    getGroupsData(groups, completed, groupsData = []) {
        if (groups.length > 0) {
    ...
    getGroupsMembersAndProjects$() {
        let groups$ = new Subject_1.Subject();
    ...
    getProjectsWithExtraData() {
        this.getGroupsMembersAndProjects$()
            .subscribe((groups) => this.getListFromGitLabServer$(this.apiUrl + '/projects', 'statistics=true')
            .subscribe((projects) => this.setProjectsDataSync$(projects, groups)
            .subscribe((projects) => {
            // let cache= new Cache()
    ...
    // }
    getUser$(user_id) {
        return Observable_1.Observable.fromPromise(rp.get(this.apiUrl + '/users/' + user_id + '/?' + this.private_token));
    }
    getUserMergeRequests$(user_id) {
    ...
    getProjectFromGitLabServer(project_id, token = this.private_token) {
        let options = {
        ...
        return new Promise((resolve, reject) => {
            rp.get(options).then(res => {
                this.getGroupsMembersAndProjects$().subscribe((groups) => this.setProjectExtraData(res, groups).then((res) => resolve(res)));
    ...
    getListFromGitLabServer$(url, params, token = process.env.GITLAB_PRIVATE_TOKEN, page_number = 1, result = [], listReady = new Subject_1.Subject()) {
        let num_in_page = 100;
        let options = {
    ...
    setProjectExtraData(project, groups) {
        project.files = new Array();
        project.contributors = new Array();
        //project.merge_requests= new Array<any>();
        project.owners = new Array();
        project.languages = {};
        project.community = new Array();
        project.clones = new Array();
        project.potential_developers = new Array();
        let projectGroup = groups.filter((group) => group.projects.findIndex((x) => x == project.id) != -1);
        projectGroup.forEach((group) => project.owners = group.users.slice(), this);
        return this.getProjectExtraData(project);
    }
    setProjectsDataSync$(projects, groups, result = [], complete = new Subject_1.Subject()) {
    ...
    getProjectMembers(project) {
        return new Promise((resolve, reject) => {
            if (project.creator_id && project.owners.indexOf(project.creator_id) == -1)
                project.owners.push(project.creator_id);
            if (project.owner && project.owners.indexOf(project.owner.id) == -1)
                project.owners.push(project.owner.id);
            this.getListFromGitLabServer$(this.apiUrl + '/projects/' + project.id + '/members', '')
                .subscribe((members) => {
                members.forEach((member) => {
                    if (project.owners.findIndex((x) => x == member.id) == -1)
                        project.owners.push(member.id);
                }, this);
    ...
    getProjectDownloadStatistics(project) {
        let options = {
    ...
    // }
    getRepositoryContainsFile(project_id, filename) {
        let options = {
    ...
    getImpersonationToken(gitlab_user_id) {
        return new Promise((resolve, reject) => {
    ...
    getStarredProjects(gitlab_user_id) {
        return new Promise((resolve, reject) => {
            this.getImpersonationToken(gitlab_user_id)
    ...
    static getGitLabAgent() {
        if (!GitLabAgent.instance)
            GitLabAgent.instance = new GitLabAgent();
        return GitLabAgent.instance;
    }
}
exports.GitLabAgent = GitLabAgent;
