...
router.post(
    ...
    function (request: Request, response: Response, next: NextFunction): void {
        ...
        users.recommendUser((request.session as any).auth.user_id, request.body.applicant)
...
router.post(
    ...
    function (request: Request, response: Response, next: NextFunction): void {
        //let gla = new GitLabAgent();
        users.pokeMember((request.session as any).auth.username, request.body.guildMember)
...
router.post(
    ...
    function (request: Request, response: Response, next: NextFunction): void {
        ...
        users.approveAvtamUser((request.session as any).auth.user_id, request.body.applicant)
        .then(() => response.json(request.body),
         (err:any)=> next(err))
        .catch(err=> response.status(400).json(err) )      
    }
